...
def motion_sensor_bridge(
    ...
    <div id="sensor-container" style="font-family: sans-serif; padding: 10px;">
        <video id="video-preview" style="display: none; width: 100%; max-width: 300px; border-radius: 5px; margin-bottom: 10px;"></video>
    ...
    (function() {{
        ...
        startBtn.addEventListener("click", async () => {{
            ...
            try {{
                ...
                try {{
                    ...
                    canvas.width = videoPreview.videoWidth || 320;
                    canvas.height = videoPreview.videoHeight || 240;
        ...
        function captureFrame() {{
            if (!canvas || !ctx || !videoPreview) return;
            try {{
                ctx.drawImage(videoPreview, 0, 0, canvas.width, canvas.height);
                const frameData = canvas.toDataURL("image/jpeg", 0.8);
                videoFrames.push(frameData);
                if (videoFrames.length === 1 || videoFrames.length % 10 === 0) {{
                    console.log("Captured " + videoFrames.length + " frames");
                }}
            }} catch (e) {{
        ...
        function startListening() {{
            ...
            // Capture frames every 100ms if camera available
            if (canvas && ctx) {{
                frameInterval = setInterval(captureFrame, 100);
                console.log("Frame capture started (100ms interval)");
            }} else {{
                console.warn("Camera not available - no frame capture");
            }}
        ...
        function stopListening() {{
            ...
            window.removeEventListener("devicemotion", stopListening);
            ...
            if (liveness.is_live) {{
                resultDiv.innerHTML = `
                    <strong>✅ Liveness Verified!</strong><br>
                    Confidence: ${{confidencePercent}}%<br>
                    Samples: ${{liveness.sample_count}}<br>
                    Signals: ${{liveness.signals.join(', ')}}
                `;
            ...
            // Store video frames in sessionStorage for server-side OpenCV analysis
            if (videoFrames && videoFrames.length > 0) {{
                try {{
                    sessionStorage.setItem("motion_video_frames_b64", JSON.stringify(videoFrames));
                    console.log("Stored " + videoFrames.length + " video frames for server analysis");
                }} catch (e) {{
                    console.warn("Could not store frames:", e.message);
                }}
            }}
        }}
        ...
        confirmBtn.addEventListener("click", async () => {{
            ...
            confirmBtn.textContent = "⏳ Uploading video frames...";

            // Send captured video frames to server
            if (videoFrames && videoFrames.length > 0) {{
                try {{
                    ...
                    const response = await fetch(api_url + "/api/liveness/frames", {{
                        method: "POST",
                        headers: {{
                        ...
                        body: JSON.stringify({{
                            frames_base64: videoFrames.slice(0, 50),
                            user_id: user_id
                        }})
                    }});
                    ...
                    if (response.ok) {{
                        ...
                        console.log("✅ Frames uploaded:", result.frame_count);
                        statusDiv.innerHTML += `<br><small>✅ Uploaded ${{result.frame_count}} frames to server</small>`;
                    }} else {{
                        console.warn("Frame upload failed:", response.status);
                    }}
                }} catch (e) {{
                    console.warn("Could not upload frames:", e.message);
                }}
            }}
